...
WITH
  ...
  SELECT
    ...
    FROM
      AGE(CURRENT_DATE,
        MAX(order_created::DATE))) AS recency,
...
SELECT
  ...
  CASE
    ...
    WHEN age > 18
  AND age <= 25 THEN 'young adult'
    WHEN age > 25 AND age <= 40 THEN 'adult'
    WHEN age > 40
  AND age <= 60 THEN 'middle-aged'
    ELSE 'senior'
END
  AS age_group,
  cd.gender,
  ...
  FROM
    AGE(CURRENT_DATE,
      cs.first_order_date)) AS customer_tenure,
  CASE
